...
public class VocabularyAnalysisPromptService {
    ...
    private String getBatchSystemPrompt() {
        return "당신은 9-13세 난독증이 있는 초등학생을 위한 어휘 분석 전문가입니다.\n" +
               ...
               "각 텍스트 블록은 특별한 구분자로 분리되어 있으며, 각 블록은 고유 ID를 가집니다.\n" +
               "## 절대적으로 중요: 예외 없이 모든 텍스트 블록마다 반드시 최소 하나 이상의 단어를 추출해야 합니다. ##\n" +
               "이 요구사항은 어떤 상황에서도 준수되어야 하며, 빈 배열이 발생하면 안됩니다.\n" +
               ...
               "- 단어 선택이 어렵더라도 반드시 각 블록 ID가 결과에 포함되어야 하며, 단어 배열이 비어있으면 안됩니다.\n\n" +
               "## 최종 확인사항: 응답에서 모든 블록 ID를 키로 가지는 JSON 객체가 생성되어야 하며, 각 키에는 최소 하나 이상의 단어 정보가 포함된 배열이 있어야 합니다. ##";
    }
